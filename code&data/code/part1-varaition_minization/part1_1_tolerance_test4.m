clear all;
clc;
% This is a test of circles and random 70 using dual-simplex method
% And change Constraint tolerance to 1e-4,1e-5,1e-6;
% construction of utur
image = imread('../../test_images/512_512_circles.png');%picture RGB
...
%1e-5
options_2 = optimoptions('linprog','Algorithm','dual-simplex');
options_2.ConstraintTolerance = 1e-5;
...
PSNR_2 = 10*log10(m*n/norm(res_x_2 - u,2));%psnr


%1e-6
options_3 = optimoptions('linprog','Algorithm','dual-simplex');
options_3.ConstraintTolerance = 1e-6;
options_3.Display = 'Iter';

tic
[res_3,y_3] = linprog(f,[],[],[Aeq_1;Aeq_2],[beq_1;beq_2],lb,ub,options_3);
toc
res_x_3 = res_3(2*kappa+1:2*kappa+m*n,1);%get x
res_i_3 = reshape(res_x_3,[m,n]);%get image
PSNR_3 = 10*log10(m*n/norm(res_x_3 - u,2));%psnr